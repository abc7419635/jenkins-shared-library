...
pipeline {
    ...
    //    failure {
    //        script {
    //            allUnrealBuildScript.buildfailure()
    //        }
    //    }
    //}
}
    ...
    node('RemoteBuildPC') {
        ...
        stage('WindowsClient Dev') {
            if(env.Build_WIN_CLIENT_DEV=='true')
            {
                ...
                build job: 'RemoteBuildCompress', parameters: [string(name: 'DATAPATH', value: env.UNREAL_BUILD_DIR+env.WindowsClientDevName),
        ...
        stage('LinuxServer Dev') {
            if(env.Build_LINUX_SERVER_DEV=='true')
            {
                ...
                string(name: 'DATAPATH', value: env.UNREAL_BUILD_DIR+env.LinuxServerDevName),
        ...
        stage('WindowsServer Dev') {
            if(env.Build_WIN_SERVER_DEV=='true')
            {
                ...
                build job: 'RemoteBuildCompress', parameters: [string(name: 'DATAPATH', value: env.UNREAL_BUILD_DIR+env.WindowsServerDevName),
        ...
        stage('LinuxServer Shipping') {
            if(env.Build_LINUX_SERVER_SHIPPING=='true')
            {
                ...
                build job: 'RemoteBuildCompress', parameters: [
                string(name: 'DATAPATH', value: env.UNREAL_BUILD_DIR+env.LinuxServerShippingName),
