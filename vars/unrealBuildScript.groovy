        ...
        stage('Increase Version') {
            if(env.SkipP4Update=='false')
            {
                bat '''
                    ...
                    set P4CLIENT=%P4WorkspaceName%

                    for /F %%i IN (D:\\_BuildTools\\temp\\ContentVersion.txt) DO (
                    set ContentVersion=%%i
                    )

                    p4 edit -c default %P4RootDir%\\Game\\ReDream\\Config\\DefaultGame.ini
                    %UNREAL_SOURCECODE_DIR%\\Engine\\Binaries\\ThirdParty\\Python\\Win64\\python.exe D:\\_BuildTools\\Python\\configReaderV2.py %BUILD_ID% %ContentVersion%
